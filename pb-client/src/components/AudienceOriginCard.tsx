...
export function AudienceOriginCard({ regions, activeViewers = 0 }: AudienceOriginCardProps) {
	const audienceRegions = useMemo(() => aggregateAudienceRegions(regions, 6), [regions]);
	const totalViewers = Math.max(activeViewers, audienceRegions.reduce((total, region) => total + region.count, 0));
			...
			{audienceRegions.length === 0 ? (
				...
				<div className="space-y-sm">
					{audienceRegions.map((region) => {
						const share = totalViewers === 0 ? 0 : Math.round((region.count / totalViewers) * 100);
